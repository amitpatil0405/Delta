...
export async function getLatestNews(category = 'All') {
    ...
    if (newsApiKey) {
      ...
      if (res.ok) {
        ...
        if (json.articles && json.articles.length > 0) {
          const formatted = json.articles.map((art, idx) => ({
            id: `api_${idx}`,
            headline: art.title,
          ...
          if (category !== 'All') {
            return {
              success: true,
              data: formatted.filter(item => item.category === category || category === 'All')
            };
          }
  ...
  // Filter fallback static news by category
  let filtered = INITIAL_NEWS_ITEMS;
  if (category && category !== 'All') {
    filtered = INITIAL_NEWS_ITEMS.filter(
